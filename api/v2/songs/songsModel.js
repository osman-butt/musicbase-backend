...
async function getSongs(artistName, albumName, songName) {
  let query = /*sql*/ `SELECT DISTINCT songs.*, artists.artistID, artists_songs.isPrimary  FROM songs 
    ...
    LEFT JOIN albums ON albums_songs.albumID = albums.albumID WHERE 1`;
...
async function getSongsById(artistName, albumName, songName, id) {
  let query = /*sql*/ `SELECT DISTINCT songs.*, artists.artistID, artists_songs.isPrimary  FROM songs 
    LEFT JOIN artists_songs ON songs.songID = artists_songs.songID 
    LEFT JOIN artists ON artists_songs.artistID = artists.artistID 
    LEFT JOIN albums_songs ON songs.songID = albums_songs.songID 
